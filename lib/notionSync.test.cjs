...
function publisherFixture() {
  ...
  return { publisher, config, pages, calls, rows, files, estimatedId, proposalSchema, fail: error => { failCreate = error; }, run: options => publisher.publishNotionProjects(readOnlyDb({ projects: rows }), config, { limit: 5, ...options }) };
}
...
function inboundFixture() {
  ...
  return { inbound, config, pages, calls, writes, files, tables, estimatedId, clientsId, proposalSchema, newPage, run: options => inbound.pullNotionChanges(writableDb(tables, writes), config, { limit: 10, ...options }) };
}
...
test('notion cron pulls inbound then publishes the safety net from the stored cursor', async () => {
  ...
  try {
    ...
    assert.deepEqual(calls[1][1], { dryRun: false, limit: 100, afterProjectId: 37 });
...
test('notion webhook pulls the changed page for our data source and ignores others', async () => {
  ...
  try {
    ...
    const ignored = await post({ type: 'page.deleted', entity: { id: 'p3' } });
    assert.equal((await ignored.json()).ignored, 'page.deleted');
