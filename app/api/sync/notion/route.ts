...
export async function POST(request: NextRequest) {
  const denied = await requireAdmin(request);
  if (denied) return denied;
  let body: { scope?: string; limit?: number; projectId?: number; afterProjectId?: number; dryRun?: boolean; [key: string]: unknown };
  try {
  ...
  const allowed = ['scope', 'limit', 'projectId', 'afterProjectId', 'dryRun'];
  if (Object.keys(body).some(key => !allowed.includes(key))) {
    return json({ error: `Only ${allowed.join(', ')} are accepted.` }, 400);
  }
  const scope = body.scope ?? 'source';
  const limit = body.limit ?? (scope === 'publish' ? 5 : 20);
  const dryRun = body.dryRun !== false;
  if (!['source', 'compare', 'setup', 'publish'].includes(scope) || !Number.isInteger(limit) || limit < 1 || limit > (scope === 'publish' ? 5 : 100) || (body.projectId !== undefined && (!Number.isSafeInteger(body.projectId) || body.projectId <= 0)) || (body.afterProjectId !== undefined && (scope !== 'publish' || !Number.isSafeInteger(body.afterProjectId) || body.afterProjectId < 0)) || (body.dryRun !== undefined && typeof body.dryRun !== 'boolean') || (!dryRun && !['setup', 'publish'].includes(scope))) {
    return json({ error: 'Use scope source/compare/setup/publish, a boolean dryRun, limit 1–100 (publish: 1–5), a positive projectId, and a nonnegative afterProjectId for publishing.' }, 400);
  }
  ...
  try {
    if (scope === 'setup') {
      return json(await setupNotionPublishing(config, dryRun));
    }
    if (scope === 'publish') {
